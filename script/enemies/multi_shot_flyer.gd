...
func _ready():
	...
	multi_shot.shot_empty.connect(func():
...
func _process(_delta):
	...
	match current_state:
		...
		state.MOVE_AND_AIM:
			if not aiming:
				aiming = true
				multi_shot.is_shot_empty = false
				multi_shot.create_shots(5, 0.25, player)
			fly_movement.move_center(_delta)
...
func destroy(without_point = false):
	...
	ReuseInitialize.to_reuse(GroupsName.MULTI_SHOT_FLYER ,self)
	stop_countdown()
	current_state = state.MOVE
