...
func create_shots(n, wait_between = 0):
	if n > 0 and i < n and is_instance_valid(parent) and not pause_shot:
		is_shot_empty = false
		...
		toxic_bullet_inst.MAX_SPEED = speed
		if not toxic_bullet_inst.get_parent():
			parent.get_parent().add_child(toxic_bullet_inst)
		toxic_bullet_inst.destory_timer.stop()
		toxic_bullet_inst.destory_timer.wait_time = 50
		toxic_bullet_inst.destory_timer.start()
