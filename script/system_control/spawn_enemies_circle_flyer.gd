...
var left_n_max = 4
var time_for_each
var flyer = preload("res://nodes/enemies/circle_flyer.tscn")
	...
	for i in range(0, left_n_max):
		attack_seq.append(func():
			if not is_first_count and total_enemies < max_enemies * 2 or is_first_count:
				var flyer_inst = ReuseInitialize.initialize(GroupsName.CIRCLE_FLYER ,flyer)
				...
				flyer_inst = ReuseInitialize.initialize(GroupsName.CIRCLE_FLYER ,flyer)
